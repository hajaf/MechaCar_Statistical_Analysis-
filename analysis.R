...
lm(formula=mpg~ vehicle_weight + spoiler_angle + ground_clearance + AWD + vehicle_length ,data= MechaCar_mpg.)
...
summary(lm(formula = mpg ~ vehicle_weight + spoiler_angle + ground_clearance +  AWD + vehicle_length, data = MechaCar_mpg.)) #generate summary statistics
...
total_summary <-  summarize(Suspension_Coil,mean(PSI),median(PSI),sd(PSI),var(PSI))
lot_summary <- Suspension_Coil%>% group_by(Manufacturing_Lot) %>% summarize(,mean(PSI),median(PSI),sd(PSI),var(PSI) )
